...
#from my_model import model_wavLM_woctc_wogrl
...
def collate_fn(batch):
    ...
    return input_mel_batch, input_lengths_batch, spkid_batch, input_wav2vec_batch, mono_file_list
...
def train(train_dataloader, output_model_dir, checkpoint_path, n_gpu=1, local_rank=0):#分布式训练，进程的编号
#def train(train_dataloader, output_model_dir, checkpoint_path):
    ...
    for epoch in range(hp.epoch_num):
        for idx, (mel_data, input_lengths, spkid, wav2vec_data, mono_file_list) in enumerate(train_dataloader):
            ...
            #mel_outputs = model(mel_data, mel_data, wav2vec_data, seq_lengths)

            loss_mel = l2_loss(mel_outputs, wav2vec_data, mask=input_mask)
            
            ctc_log_probs = nn.functional.log_softmax(ctc_logits, dim=2).permute(1, 0, 2) 
            loss_ctc = ctc_loss(ctc_log_probs, batch_mono_list, ctc_input_lengths, batch_mono_lengths)
            ctc_loss_weight = step_weight_decay(hp.loss_ctc, step)

            loss = loss_mel + vq_loss * hp.loss_weight_vq+ loss_ctc * ctc_loss_weight
            loss.backward()
            ...
            local_log_value(local_rank, "loss_ctc", loss_ctc, step)
            local_log_value(local_rank, "ctc_loss_weight", ctc_loss_weight, step)

            if step % hp.log_interval == 0 and step > 0:
                runtime = datetime.now().timestamp() - start_time
                print("lr is {0}".format(current_lr))
                log_string = "step {0}, loss {1:.8f}, lr {2:.8f}, vq_loss {3:.8f}, ppl {4:.8f}, loss_ctc {5:.8f}, {6:.3f}".format(step, loss, current_lr, vq_loss, vq_perplexity, loss_ctc, runtime)
...
def main():
    ...
    parser.add_argument('--name', default='wavLM+vq+ctc')
    parser.add_argument('--input', default='/ssdhome/wl/data/vctk_vad_wav/wavLM/wavLM_ctc.txt')
    ...
    parser.add_argument('--gpus',default=5,help="gpu ids, eg. 0,1,2,3")
    ...
    sampler = feat_dataset_copy.RandomIdentitySampler(dataset=train_dataset, num_instances=1)
    ...
    train(train_dataloader, output_model_dir, checkpoint_path, args.num_gpu, args.local_rank)
