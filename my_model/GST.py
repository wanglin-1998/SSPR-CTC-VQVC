...
class GST(nn.Module):
    ...
    def forward(self, inputs, seq_lengths=None):
        enc_out = self.encoder(inputs, seq_lengths)
        style_embed = self.stl(enc_out)
        return style_embed   ####torch.Size([16, 1, 256])
...
class ReferenceEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        # self.bns = nn.ModuleList([nn.InstanceNorm2d(num_features=hp.ref_enc_filters[i]) for i in range(K)])

        #out_channels = self.calculate_channels(hp.num_mels, 3, 2, 1, K)
        self.gru = DynamicGRU(input_size=128,
                              hidden_size=hp.E // 2,
                              batch_first=True)
    ...
    def forward(self, inputs, seq_lengths=None):
        ...
        for idx, (conv, bn) in enumerate(zip(self.convs, self.bns)):
            ...
            if (idx + 1) % 2 == 0:
                out = F.avg_pool2d(out, kernel_size=(2, 4), stride=(2, 4))
        ...
        input_lengths = None if seq_lengths is None else seq_lengths // 8
        memory, out = self.gru(out, input_lengths, flatten_params=True)  # out --- [1, N, E//2]
